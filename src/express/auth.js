...
export function auth(options = {}) {
  ...
  return async function rbacAuth(req, res, next) {
    try {
      ...
      req.session = session;
      setContext(req, {adapter, auth: authCore, rbac, session});
      return next();
    } catch (error) {
...
async function authenticateBasic({ header, authCore, adapter, createSession = true }) {
  ...
  if (createSession !== false) {
    ...
    if (activeSession) return authCore.getSession(activeSession.id);
    return authCore.login({ username, password, options: {} });
  }
  return {id: null, user: publicUser(user), options: {}};
}

async function getExistingSession(adapter, userId) {
  if (typeof adapter.findActiveSessionByUserId !== "function") return null;
